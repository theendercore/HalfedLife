...
class ModelProvider(output: FabricDataOutput) : FabricModelProvider(output) {
    ...
    override fun generateBlockStateModels(gen: BlockStateModelGenerator) {
        try {
            ...
            val doubleProperty = BlockStateVariantMap.create(WallBlock2x1.RIGHT)
            val dBlock = HLBlocks.GRAFFITI_YELLOW_2X1

            val modelLeft = gen.wallBlock1x1Model(dBlock, "_left")

            val modelRight = gen.wallBlock1x1Model(dBlock, "_right")

            gen.blockStateCollector.accept(
                VariantsBlockStateSupplier.create(dBlock)
                    .coordinate(createSouthDefaultHorizontalRotationStates())
                    .coordinate(
                        doubleProperty
                            .register(false, BlockStateVariant.create().put(VariantSettings.MODEL, modelLeft))
                            .register(true, BlockStateVariant.create().put(VariantSettings.MODEL, modelRight))
                    )
            )
    ...
    private fun BlockStateModelGenerator.wallBlock1x1Model(block: Block, suffix : String, x: Boolean): Identifier {
       ...
       return Model(id("block/wall_block_1x1").myb, Optional.empty(), ketText)
            .upload(
